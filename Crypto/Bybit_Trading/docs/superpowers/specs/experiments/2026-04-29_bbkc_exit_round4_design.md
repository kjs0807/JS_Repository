...
- BASELINE (cell == "F0")
- UNKNOWN (F0 없이 부분 실행)
- STRONG_PROMOTE / PROMOTE / NEUTRAL / KILL — Δwf_oos+, Δr, ΔDD 기반
...
def integrate_label(cell_id: str, by_sym: dict[str, dict]) -> str:
    ...
    if eth_promote:
        if not has_kill and not has_unknown_or_warning:
...
| 2 | ROBUST_PROMOTE | ETH ∈ {SP, P} AND BTC, AVAX 모두 KILL/UNKNOWN/warning 아님 | 공통 BBKC exit 후보 |
| 3 | ETH_ONLY_PROMOTE | ETH ∈ {SP, P} AND BTC 또는 AVAX 중 하나 KILL | ETH 전용 운영 후보 |
| 4 | ETH_PROMOTE_MIXED | ETH ∈ {SP, P} AND (BTC 또는 AVAX에 UNKNOWN/warning 존재 AND KILL 없음) | 추가 검증 필요 |
| 5 | DAMAGING | ETH ∉ {SP, P} AND BTC 또는 AVAX 중 하나 KILL | 폐기 |
| 6 | NO_SIGNAL | 그 외 (ETH 이득 없음, 다른 심볼 손해도 없음) | 무시 |
...
### 7.1 기대값 (라운드 3 §15.1에서)

```
wf_oos_positive: 6
mean_r_per_trade: +0.064
trade_count: 154
```
...
## Per-Symbol × Distance Heatmaps (3×3 grid of be × start, label per cell)
...
- `python -m scripts.bbkc_exit_eval --smoke` 정상 종료 + 출력 4개 파일 생성
- be30_st60_di30 × BTCUSDT × window 0 단일 셀로 빠른 검증
...
## 14. Round 4 Results (sweep 후 채울 placeholder)

**Status**: TBD

### 14.1 판정 결과

(per-cell 통합 라벨 분포 + per-symbol heatmap 요약)

### 14.2 핵심 발견

- TF_early가 robust 영역인가? (Yes/Partial/No)
- 어느 trail 파라미터 영역이 가장 강한가?
- ROBUST_PROMOTE / ETH_ONLY_PROMOTE / DAMAGING / NO_SIGNAL 분포
